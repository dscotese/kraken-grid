...
export const AllocCon = (config, assets = [{ ticker: 'ZUSD', target: 1 }]) => {
    ...
    async function findRange(ticker, ppct, today = false) {
        ...
        if (pair > '') {
            ...
            prices.forEach(a => {
                ...
                if (process.TESTING === 'findRange' && pHB + pSL > 0)
                    console.log(339, { SL, HB, pSL, pHB });
    ...
    async function adjust(ticker, apct, ppct) {
        ...
        if (!already)
            throw Error(`${ticker} not found in portfolio.`);
    ...
    */
    ...
    async function bestTrade(current, inTkr = '', FullValue = 1000) {
        ...
        if (assets[0].ticker !== pnum)
            throw new Error("First asset must be Numeraire!");
        if (size() < 3)
            tkr = assets[1].ticker;
        if (tkr > '') {
            ...
            await Promise.all(assets.map(async (a) => {
                ...
                if (typeof (c) !== 'undefined') {
                    ...
                    console.log(a.ticker, `\t${pct(c.target)}`, `\t${pct(atargs[a.ticker])}`, `\t${pct(del)}`, bot.getTickers().includes(a.ticker) ? '' : '(-> cash)', ranges[a.ticker] ? ranges[a.ticker] : '');
                    if (!bot.getTickers().includes(a.ticker))
                        priorities[0].d += del;
                }
            }));
        ...
        while (pair === '' && ![typeof (tooMuch), typeof (notEnough)].includes('undefined')) {
            ...
            if (pair !== '') {
                ...
                if (buysell === '')
                    throw Error(`${pair} didn't match ${tooMuch.t} or ${notEnough.t}`);
        ...
        if (sumTotal > portfolio.limits[1] && portfolio.limits[1] !== -1) {
            ...
            console.log(sumTotal, "is more than", `${portfolio.limits[1]}, so I'm lowering the amount to`, amt);
        }
        if (buysell && pair && price && amt)
            console.log('Trade:', `${buysell + pair} ${price} ${amt}`);
        else
            console.log("226: No trade could be made [buysell,pair,price,amt].", [buysell, pair, price, amt]);
        if (pending > '') {
            ...
            if (typeof (found) === 'undefined') {
                ...
                console.log("I found", found[0], "in Open Orders.");
        }
        // if(pending == '') 
    ...
    function addAsset(ticker, target) {
        ...
        if (target < 0)
            throw new Error('Cannot allocate a negative amount.');
        if (!already) {
    ...
    async function list(compare = false) {
        ...
        await Promise.all(assets.map(async (a) => {
            ...
            if (compare) {
                ...
                str = `${str}\n${a.ticker}\t${t}%\t${atd}%\t${a.adjust ? ranges[a.ticker].map(price7) : '\t'}\t${a.adjust ? `(${a.adjust.join(',')})` : '\t'}\t${pct(ctarg)}%\t${trade}`;
            }
            else
                str = `${str}\n${a.ticker}\t${t}%\t${atd}%`;
        }));
    ...
    function setNumeraire(ticker) {
        ...
        if (old !== -1) {
            ...
            assets.unshift({ ticker, target: 0 });
    ...
    async function getAllocation(desired = false, refresh = true) {
        ...
        const ret = await total.getAlloc(portfolio.Numeraire || 'ZUSD', bot.numerairesFromPairs());
    ...
    async function setRanges(tickersLH) {
        ...
        await Promise.all(Object.entries(tickersLH).map(async (t) => {
            ...
            if (rt) {
                ...
                if (moved)
                    console.log("Range for", tk, "updated: ", ranges[tk]);
    ...
    return Promise.all(assets.map(async (a) => {
        if (a.adjust)
            await adjust(a.ticker, a.adjust[0], a.adjust[1]);
        else
            atargs[a.ticker] = a.target;
    })).then(() => ({ setNumeraire, list, addAsset, bestTrade, save,
