...
interface ManagerInstance {
    catcher(line: number, err: Error): void;
    doCommands(cl?: string[]): Promise<boolean>;
...
interface ManagerConstructor {
    (config: ManagerConfig): ManagerInstance;
}

const prompt = PSCon({sigint: true});
 
function Manager(config: ManagerConfig):ManagerInstance {
    if( config.man ) return config.man;
    
    let web;
    let cmdList:String[] = [];
    let auto: NodeJS.Timeout | undefined = undefined;
    let delay = 60;
    let portfolio;
    let report;
...
[{'date': '2024-01-01', 'cost': 1000.00,'amount': 0.05},
 {'date': '2024-06-01', 'cost': 1000.00,'amount': 0.02}]`);
    ...
    async function doCommands(cl=cmdList): Promise<any> {
        ...
        while(cdx < cmds.length) try {
	    	...
	    	// eslint-disable-next-line no-await-in-loop
                ...
                clearInterval(auto);
                if(args[1]&&!isNaN(Number(args[1]))) delay = Number(args);
    ...
    function listener() {
        let cmd = '';
        let waiter: NodeJS.Timeout | undefined;
        // clearInterval(auto);
        let data = '';
        while(data !== null) {
            cmd += data;
