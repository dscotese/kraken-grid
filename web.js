...
function Web(config) {
    ...
    const tkrs = []; // Associative array of totals for ticker indices.
    ...
    function head(whatElse = '') {
        return `<!DOCTYPE html><head>${getJQ()}<script type='module' src='/js/md-block.js'></script>\n`
    ...
    app.get('/', async (req, res, next) => {
        logged = ""; // "<!-- " + {req,res,next} + " -->";
        res.write(`${head() + Documentation()}<div id='GDiv'><canvas id='myCanvas'>`
    ...
    function weblink(tkr) {
        if (tkr === portfolio.Numeraire)
            return portfolio.Numeraire;
        return `<a href='https://trade.kraken.com/charts/KRAKEN:${tkr[0] === 'X' && tkr.length === 4
            ? tkr.substr(1) : tkr}-${portfolio.Numeraire.substr(1)}' target='chart'>${tkr}</a>`;
    }
    function AssetsTable() {
        ...
        let ret = "";
        const rows = []; // rows is an associative array of object arrays.
        let tkr;
        let amt;
        let ki;
        let label;
        let sav; // ktks is "Known Tickers"
        rows[''] = [{ key: 'AAAA', val: `<th title='Total Value'>${sigdig(total, 10, 2)}</th>` }].concat(ktks.map((e) => ({ key: e, val: `<th>${weblink(e)}</th>` })));
        ...
        for (let h = 0; h <= Savs.length; h += 1) {
            ...
            sav = h === Savs.length
                ...
                : Savs[h];
            label = sav.label;
            assets = sav.assets.sort((a, b) => a.ticker < b.ticker ? -1 : 1);
            ...
            for (let a = 0; a < assets.length; a += 1) {
                ...
                if (isNaN(tkrs[tkr])) {
                    ...
                    tkrs[tkr] += amt; // Totals for ticker indices.
            }
        }
    ...
    app.get('/data', async (req, res) => {
        ...
        // ------------------------------------
        await Promise.all(Object.keys(tkrs).filter((k) => !Object.keys(tt).includes(k))
            .map(async (t) => {
